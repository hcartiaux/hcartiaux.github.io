...
You can request a peering session with the nodes below via SSH on port `4242` (provided that [your `mntner` object in the dn42 registry contains at least one `auth` attribute with a ssh public key of type `ssh-ed25519` or `ssh-rsa`](https://dn42.eu/howto/Registry-Authentication)).
